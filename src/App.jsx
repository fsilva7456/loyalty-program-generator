  ...
  const renderDriverAnalysis = (driverKey) => {
    ...
    const evaluation = programData.analysis[`${driverKey}Evaluation`];
    
    return (
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-2xl font-semibold mb-6">{driver.name} Driver Analysis</h2>
        ...
        <div className="space-y-6">
          {Object.entries(evaluation.subDriverAnalysis).map(([key, analysis]) => {
            const subDriver = driver.subDrivers[key];
            return (
              <div key={key} className="p-4 border rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h4 className="text-lg font-semibold">{subDriver.name}</h4>
                    <p className="text-sm text-gray-600">{subDriver.description}</p>
                  </div>
                  <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full ml-4 shrink-0">
                    {analysis.score}/10
                  </span>
                </div>
